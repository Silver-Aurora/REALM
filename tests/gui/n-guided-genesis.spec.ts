...
test.describe("N. 司卷问答 · 对话式引导创世", () => {
  test("N1 全程引导：八步问答、已定之卷浮现、落笔入界进入对局", async ({ page }) => {
    ...
    await expect(page.locator(".guided-question")).toContainText("先给这个世界起个名字吧");

    // 第一步必填：空输入时「落墨」禁用。
    await expect(page.locator(".guided-confirm")).toBeDisabled();

    // 世界之名。
    await page.getByRole("textbox", { name: "世界之名输入" }).fill(worldName);
    ...
    await expect(scroll).not.toContainText("纪元基调");
    ...
    // 世界底色：输入一句题跋。
    await page.getByRole("textbox", { name: "世界底色输入" }).fill("云海上的旧船港，船员用风声记账。");
    await page.locator(".guided-confirm").click();
    await expect(
    ...
    // 故事开篇：标题 + 缘起。
    await page.getByRole("textbox", { name: "故事开篇输入" }).fill("雾中航船");
    await page.getByRole("textbox", { name: "故事缘起输入" }).fill("一艘无籍船在雾夜靠岸。");
    await page.locator(".guided-confirm").click();
    ...
    await page.getByRole("textbox", { name: "你的身份输入" }).fill("替港口辨认风声的新账房");
    await page.locator(".guided-confirm").click();
    await expect(
      scroll.locator(".scroll-entry", { hasText: "新账房" }),
    ).toBeVisible();

    // 你的姿态（批次 S 新增步）：留白跳过 = 默认入局。
    await expect(page.locator(".guided-heading h2")).toHaveText("你的姿态");
    await page.locator(".guided-skip").click();

    // 同行之人：留白独行（空阵容由旁白推进，既有能力）。
    // AI 代笔候选点选在 N2 以确定性路由拦截覆盖。
    await page.locator(".guided-skip", { hasText: "留白独行" }).click();

    // 初始场景：全部留白（不写默认值，留给设定结晶生长）。
    await page.locator(".guided-skip", { hasText: "留白" }).click();
    await expect(scroll).not.toContainText("初始场景");

    // 合卷总览：已定内容齐备，一键入界。
    await expect(page.locator(".guided-question")).toContainText("最后检查一遍");
    ...
    await expect(review).not.toContainText("纪元基调");
  ...
  test("N3 AI 代笔：候选点选即定并写入已定之卷（确定性拦截）", async ({ page }) => {
    ...
    await expect(
      page.getByRole("textbox", { name: "世界之名输入" }),
    ).toHaveValue("汐雾集");
    await page.locator(".guided-confirm").click();
    const scroll = page.locator(".guided-scroll");
    ...
    await page.locator(".guided-skip", { hasText: "就这些" }).click();
