...
> 本文说明 REALM 当前 Web 预览采用的系统结构、数据流和设计取舍。它以现有代码、数据库迁移和运行时契约为准；未来规划不会写成已完成能力。
...
REALM 当前是本地或受控环境中的单用户 Web 预览。它不是托管 SaaS，也不把公网账号、计费、备份和多人运营能力伪装成已经完成的功能。
...
Worldline 不是额外的一层会话。它回答的是“这件事发生在哪条因果历史上”，而不是“用户现在打开了哪个页面”。

## 3. 一次行动如何变成世界状态

一次用户行动不会直接变成一条模型生成的“事实”。它会经过以下路径：
...
服务端不会把客户端传来的世界、Record 或角色身份当作可信事实。它会根据当前会话和服务端数据重新确定：
...
这一步先于模型调用。模型不能通过提示词改变自己的 Workspace、角色身份或可见范围。
...
控制面数据不会因为“可能有用”就进入角色上下文。未提交的候选、过滤统计、数据库连接、API key 和其他角色的私密信息都不属于模型输入。
...
这样，角色在新 Record 中仍能继承已经形成的连续记忆，同时不会把一个 Record 的局部状态误当成角色的永久属性。
...
因此，角色的“记忆”不是把完整聊天记录塞回 Prompt。它是一组带来源、时间、观察者和状态的可检索结论。
...
顺序不能反过来。先做相似度搜索、最后再删秘密，会留下越权读取的风险。
...
这些数据都带有来源、时间或 Worldline 边界。世界知识不是一张会被模型随意覆盖的长文本；文章、关系和检索索引都可以从更小的事实重新生成或校验。
...
这些派生任务不会替代正式 Event，也不会在后台静默改写已经确认的历史。
...
回溯不是把数据库改回旧版本。新的行动先在因果覆盖层中运行，系统在升格时检查它与现有未来之间的冲突：

- 没有硬冲突时，可以继续形成新的正式历史；
- 发生硬冲突时，创建新的 Worldline 分支；
- 已经提交的未来 Record 不会被静默替换。

这让“重新走一次过去”成为可追踪的历史分支，而不是对原记录做不可见的覆盖。
...
pgvector 用于记忆和世界知识的检索路径。向量、摘要、Redis 缓存和模型供应商缓存都不是事实来源。
...
- 应用角色不能绕过 RLS，也不能拥有数据库、创建角色、删除历史或关闭保护触发器的权限。
...
模型供应商不获得数据库凭据，也不能直接访问内部 Repository。API key 只保存在本机配置中，不进入 Git、日志、事件或普通模型上下文。
...
页面读取的是服务端投影，不直接读数据库。SSE 发送新事件提示，客户端再按版本和权限刷新完整 Record envelope；断线重连不会依赖浏览器内存里的临时状态来“猜”历史。

本地默认只绑定 `127.0.0.1`。Docker 模式使用本地 pgvector 容器，数据库端口也只发布到 loopback。启动器会处理常见端口占用，但不会自动把服务暴露到局域网或公网。

## 9. REALM 的优势

这些优势来自系统结构本身，不依赖模型宣传词。

### 9.1 长期连续性有数据基础

世界、故事、Record、Worldline、角色连续体和记忆不是一段 Prompt 里的标签，而是可查询、可验证、带来源的数据。角色跨 Record 继续存在时，系统可以区分“角色长期知道的事”和“某个场景临时看到的事”。
...
系统不是先把所有内容交给模型，再试图从回答中删掉秘密。角色上下文在读取和编译阶段就按时间、Worldline、受众和认知范围收窄。这个顺序更容易审计，也更容易做回归测试。

### 9.4 历史可以回读和恢复

Append-only Event、Record head、幂等键、Context snapshot 和 Outbox 让一次行动留下完整链路。服务中断后可以从稳定状态恢复；发生修正时追加新事实，不需要偷偷改写旧历史。

### 9.5 规则与模型可以各自演进

规则工具不绑定某一家模型，模型也不需要知道数据库表结构。换 provider、升级提示词或替换检索算法时，正式提交和权限边界仍然由服务端掌握。

### 9.6 本地运行降低了使用门槛和数据暴露面

Web、PostgreSQL、pgvector 和启动器可以在同一台电脑上完成。用户不需要先申请托管服务，也不会因为默认启动就把世界数据放到公共服务器。需要远程模型时，只有生成所需的最小上下文会离开本机。

### 9.7 复杂性集中在系统内部

用户看到的是世界、故事、记录和角色；时间游标、受众快照、记忆召回、事务锁和 Outbox 由运行时处理。系统内部保持严格边界，界面不需要把这些实现细节变成一排排开关。

## 10. 当前边界

REALM 目前仍是 Web 自托管预览，以下内容没有被本文当作已完成能力：

- 公网账号、多租户运营、计费和托管服务；
- 完整的公网多人部署与跨设备网络验收；
- 自动更新、签名安装包和正式发布渠道；
- 所有模型提供方的统一质量、延迟和价格；
- 把任意外部角色卡、世界书或工具定义直接变成可执行代码；
- 用后台推断替代用户对高层 Canon 的确认。

这些边界是产品状态的一部分。系统设计先保证可回读、可验证和可恢复，再逐步增加规模和自动化。
